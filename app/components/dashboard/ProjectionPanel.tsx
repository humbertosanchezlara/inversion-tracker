  ...
  return (
    <section className="flex min-h-[360px] flex-col gap-3 rounded-2xl border border-[var(--hairline)] bg-[var(--panel-bg)] px-5 py-[18px] backdrop-blur-2xl">
      <div className="flex flex-col justify-between gap-3 md:flex-row md:items-start">
        <div>
          <p className="font-mono text-[10px] uppercase tracking-[0.16em] text-[var(--muted)]">Proyección · 30 años</p>
        ...
        <div className="flex w-fit gap-1.5 rounded-lg border border-[var(--hairline)] bg-black/25 p-0.5">
          {["10y", "15y", "20y", "25y", "30y"].map((item) => (
            <span
              className={`rounded-md px-2.5 py-1 font-mono text-[10px] ${
                item === "30y" ? "bg-white/[0.08] text-[var(--foreground)]" : "text-[var(--muted)]"
              }`}
              key={item}
            >
              {item}
      ...
      <div className="min-h-[205px] flex-1">
        <ResponsiveContainer height="100%" minHeight={205} width="100%">
          <AreaChart data={chartData} margin={{ bottom: 2, left: 0, right: 10, top: 10 }}>
            <defs>
