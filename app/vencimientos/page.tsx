...
export default function VencimientosPage() {
  return <PlaceholderView eyebrow="Vencimientos" title="Timeline y tabla filtrable" />;
}

function PlaceholderView({ eyebrow, title }: { eyebrow: string; title: string }) {
  return (
    <main className="flex min-h-screen items-center justify-center bg-[var(--background)] px-6 text-[var(--foreground)]">
      <section className="w-full max-w-xl rounded-2xl border border-[var(--hairline)] bg-[var(--panel-bg)] p-8 backdrop-blur-2xl">
        <p className="font-mono text-[10px] uppercase tracking-[0.16em] text-[var(--muted)]">{eyebrow}</p>
        <h1 className="mt-2 text-[24px] font-semibold tracking-[-0.015em]">{title}</h1>
        <p className="mt-3 text-[13px] leading-6 text-[var(--text-soft)]">
          Vista pendiente de handoff. El dashboard principal ya enlaza aquí para reservar la navegación.
        </p>
