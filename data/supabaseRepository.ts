...
function fromLot(row: Record<string, unknown>): InvestmentLot {
  return {
    ...
    date: row.investment_date ? String(row.investment_date) : undefined,
    maturityDate: row.maturity_date ? String(row.maturity_date) : undefined,
...
function toLot(userId: string, lot: InvestmentLot) {
  return {
    ...
    user_id: userId,
    month: lot.month,
    investment_date: lot.date ?? null,
    maturity_date: lot.maturityDate ?? null,
...
function fromAssetMovement(row: Record<string, unknown>): AssetMovement {
  return {
    ...
    quoteCurrency: optionalText(row.quote_currency) ?? "MXN",
...
function toAssetMovement(userId: string, movement: AssetMovement) {
  ...
  return {
    ...
    unit_price: isNone ? null : movement.unitPrice ?? null,
    amount: isNone ? null : movement.amount ?? null,
    quote_currency: movement.quoteCurrency,
